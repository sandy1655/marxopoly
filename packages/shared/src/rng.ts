...
export function nextInt(state: number, minInclusive: number, maxInclusive: number): RandomDraw<number> {
  const { value, state: next } = nextRandom(state);
  const span = maxInclusive - minInclusive + 1;
  return { value: minInclusive + Math.floor(value * span), state: next };
}

export function rollDice(state: number): RandomDraw<[number, number]> {
...
export function shuffle<T>(items: readonly T[], state: number): RandomDraw<T[]> {
  const out = [...items];
  let s = state;
