...
const collect = (amount) => ({ kind: 'cash', amount });
/** Pay `amount` to the bank. */
const pay = (amount) => ({ kind: 'cash', amount: -amount });
/** Every other player pays you `amount`. */
const collectFromEveryone = (amount) => ({ kind: 'collect_each', amount });
/** You pay every other player `amount`. */
const payEveryone = (amount) => ({ kind: 'pay_each', amount });
...
const card = (id, text, effect) => ({ id, text, effect });
const buildDeck = (deck, rows) => rows.map((row) => ({ ...row, deck }));
// ---------------------------------------------------------------------------
// The cards
// ---------------------------------------------------------------------------
export const FORTUNE_CARDS = buildDeck('fortune', [
...
export const LEDGER_CARDS = buildDeck('ledger', [
...
function assertUniqueIds(cards) {
    ...
    for (const c of cards) {
        if (seen.has(c.id))
            throw new Error(`cards.ts: duplicate card id "${c.id}" — ids must be unique`);
        seen.add(c.id);
    }
}
